...
<div className="ml-2 mr-2 bg-gradient-to-br from-gray-50 via-white to-gray-100 ">
  <section ref={containerRef} className="px-4 mx-auto py-14 max-w-7xl">
    <h3 className="mb-16 text-3xl font-bold text-center text-black">Our Approaches</h3>

    <div className="grid max-w-6xl gap-8 mx-auto md:grid-cols-3 lg:grid-cols-5">
      {ITEMS.map((item, idx) => (
        <div
          key={item.title}
        ...
        >
          <div className="p-6 hover:shadow-xl group">
            <img
              src={item.src}
              alt={item.title}
              className="w-full h-auto mx-auto transition-transform duration-300 group-hover:scale-110"
            />
            {/* <h4 className="mt-4 text-base font-semibold text-center text-brandNavy group-hover:text-orange-500">
              {item.title}
            </h4> */}
          </div>
        </div>
